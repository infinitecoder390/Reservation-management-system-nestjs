...
// Common abstract repository to use commonly all over microservices
export abstract class AbstractRepository<TDocument extends AbstractDocument>{
    protected abstract readonly logger:Logger
    constructor(protected readonly model:Model<TDocument>){}
    async create(document:Omit<TDocument,'_id'>):Promise<TDocument>{
        const createDocument = new this.model({...document,_id:new Types.ObjectId()});
        const response = (await createDocument.save()).toJSON() as unknown as TDocument
        return response;
    }
